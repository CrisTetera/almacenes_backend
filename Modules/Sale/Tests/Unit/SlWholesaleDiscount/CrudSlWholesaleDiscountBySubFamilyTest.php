...
class CrudSlWholesaleDiscountBySubFamilyTest extends TestCase
{
    ...
    private function storeWholesaleDiscounts() {
        return $this->json('POST', 'api/sl_wholesale_discount_subfamily', $this->tempJson);
    }
    ...
    private function updateWholesaleDiscount($id) {
        return $this->json('PUT', "api/sl_wholesale_discount_subfamily/$id", $this->tempJson);
    }
    ...
    public function test_ShouldStoreWholesaleDiscounts()
    {
        ...
        // Assert sl wholesale discount subfamily
        foreach( $this->tempJson['discount_schema'] as $data )
        {
            $this->assertDatabaseHas('sl_wholesale_discount_subfamily', [
     ...
     * @return void
     */
    public function test_ShouldThrowException_IfSubFamilyDoesntExists_WhenUpdate()
    {
    ...
    public function test_ShouldUpdateWholesaleDiscount()
    {
        ...
        $response = $this->updateWholesaleDiscount(3); // Id de la subfamilia
        ...
        // Assert sl wholesale discount subfamily deleted
        foreach( $oldJson['discount_schema'] as $data )
        {
            $this->assertDatabaseHas('sl_wholesale_discount_subfamily', [
                'wh_subfamily_id' => $oldJson['wh_subfamily_id'],
