...
use Modules\General\Http\Requests\GUser\RecoverBarAuthCodeRequest;
use Modules\General\Services\GUser\BarAuthCodeService;
use Modules\General\Services\GUser\CrudUserService;
use Modules\General\Http\Requests\GUser\CreateGUserRequest;
use Modules\General\Http\Requests\GUser\EditGUserRequest;
use Modules\General\Filters\GUser\FiltersGUserPerRole;
use Modules\General\Filters\GUser\FiltersGUserRut;
use Spatie\QueryBuilder\Filter;
use Illuminate\Support\Facades\DB;
use App\Http\Response\CustomResponse;
use Modules\General\Filters\GUser\FiltersGUserSearch;
...
class GUserController extends Controller
{
    ...
    {
        $query =  QueryBuilder::for(GUser::where('g_user.flag_delete', 0))
                            ->allowedIncludes('hr_employee')
                            ->allowedFilters(
                                'names',
                                'last_name1',
                                'last_name2',
    ...
    {
        return QueryBuilder::for(GUser::where('id', $idGUser))
                            ->allowedIncludes('hr_employee')
                            ->allowedAppends('commissions', 'discount', 'is_ground_seller', 'customers')
                            ->first();
    }
