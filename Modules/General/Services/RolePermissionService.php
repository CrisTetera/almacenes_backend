...
class RolePermissionService
{
    ...
    public function isAuthorized(Request $request)
    {
        ...
        try
        {
            $user = GUserPos::with('hrEmployee')->where('auth_code', $request->auth_code)->where('flag_delete', false)->firstOrFail();
            ...
            return CustomResponse::ok([
                ...
                'user' => [
                    'id' => $user->id,
                    'firstnames' => $userArray['hr_employee']['names'],
    ...
    public function isBarAuthorized(Request $request)
    {
        ...
        try
        {
            $user = GUserPos::with('hrEmployee')->where('bar_auth_code', $request->auth_code)->where('flag_delete', false)->firstOrFail();
            if (!$user || !$request->auth_code) {
                throw new ValidationException("No tienes los permisos para realizar esta operación");
            }
            $userArray = $user->toArray();
